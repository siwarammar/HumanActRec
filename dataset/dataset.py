...
class HMDB51_test(Dataset):
    ...
    def __init__(self, train, opt, split=None):
        ...
        #print("lab names : ", self.lab_names)
        # Number of classes
...
class UCF101_test(Dataset):
    ...
    def __init__(self, train, opt, split=None):
        ...
        for line in f:
            ...
            if os.path.exists(os.path.join(self.opt.frame_dir, line)) == True:
                self.data.append((os.path.join(self.opt.frame_dir, line), class_id))
    ...
    def __getitem__(self, idx):
        ...
        if self.train_valtest == 0: 
            clip = get_test_video(self.opt, frame_path, Total_frames)
        else:
            clip = get_train_video(self.opt, frame_path, Total_frames)
                    
        return((scale_crop(clip, self.train_valtest, self.opt), label_id))
