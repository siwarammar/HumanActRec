...
def generate_model( opt):
    assert opt.model in ['swin-t','swin-s', 'swin-b', 'swin-sth', 'resnext']
    if opt.model=='swin-t':
        ...
        linear_layer = nn.Linear(6144, opt.n_classes).cuda()
        

    elif opt.model =='swin-b':
        net =  SwinTransformer3D(depths=[2, 2, 18, 2], embed_dim=128, num_heads=[4, 8, 16, 32], num_classes=400)
        linear_layer = nn.Linear(32768, opt.n_classes).cuda()   #32768 #8192 #16384
