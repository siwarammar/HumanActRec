...
def generate_model( opt):
    assert opt.model in ['swin']

    net = SwinTransformer3D(num_classes=400)
    

    net = net.cuda()
    net = nn.DataParallel(net)
    ckpt = torch.load("/l/users/siwar.ammar/MARS/swin_tiny/swin_tiny_patch244_window877_kinetics400_1k.pth")
    ckpt_dict = load_weights(ckpt['state_dict'])
    net.load_state_dict(ckpt_dict,strict=False)
    model = nn.Sequential(net, nn.Linear(98304, opt.n_finetune_classes).cuda())
